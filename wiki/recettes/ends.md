## Assorted Salad
**ID Technique** : `ends_delight:assorted_salad` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Crimson Fungus | ![crimson_fungus](../images/ends_crimson_fungus.png) |
| - | Warped Fungus | ![warped_fungus](../images/ends_warped_fungus.png) |
| - | Crops/Cabbage | ![crops/cabbage](../images/ends_crops/cabbage.png) |
| - | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| - | Dragon Egg Shell | ![dragon_egg_shell](../images/ends_dragon_egg_shell.png) |

---
## Bubble Tea
**ID Technique** : `ends_delight:bubble_tea` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Milk | ![milk](../images/ends_milk.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |
| - | Ender Pearl Grain | ![ender_pearl_grain](../images/ends_ender_pearl_grain.png) |

---
## Chorus Cookie
**ID Technique** : `ends_delight:chorus_cookie` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Wheat | ![wheat](../images/ends_wheat.png) |
| - | Wheat | ![wheat](../images/ends_wheat.png) |
| - | Chorus Fruit Grain | ![chorus_fruit_grain](../images/ends_chorus_fruit_grain.png) |

---
## Chorus Flower Pie
**ID Technique** : `ends_delight:chorus_flower_pie` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Dough | ![dough](../images/ends_dough.png) |
| - | Dried Chorus Flower | ![dried_chorus_flower](../images/ends_dried_chorus_flower.png) |
| - | Dried Chorus Flower | ![dried_chorus_flower](../images/ends_dried_chorus_flower.png) |
| - | Poppy | ![poppy](../images/ends_poppy.png) |
| - | Sugar | ![sugar](../images/ends_sugar.png) |

---
## Chorus Flower Tea
**ID Technique** : `ends_delight:chorus_flower_tea` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Ghast Tear | ![ghast_tear](../images/ends_ghast_tear.png) |
| - | Dried Chorus Flower | ![dried_chorus_flower](../images/ends_dried_chorus_flower.png) |

---
## Chorus Fruit Crate
**ID Technique** : `ends_delight:chorus_fruit_crate` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |

---
## Chorus Fruit
**ID Technique** : `minecraft:chorus_fruit` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit Crate | ![chorus_fruit_crate](../images/ends_chorus_fruit_crate.png) |

---
##  4}]
**ID Technique** : `[{'item': 'ends_delight:chorus_fruit_grain', 'count': 4}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |

---
## Chorus Fruit Milk Tea
**ID Technique** : `ends_delight:chorus_fruit_milk_tea` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Milk | ![milk](../images/ends_milk.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |

---
## Chorus Fruit Pie
**ID Technique** : `ends_delight:chorus_fruit_pie` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Pie Crust | ![pie_crust](../images/ends_pie_crust.png) |
| 1 | Wheat | ![wheat](../images/ends_wheat.png) |
| 2 | Poppy | ![poppy](../images/ends_poppy.png) |
| 3 | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| 4 | Chorus Flower | ![chorus_flower](../images/ends_chorus_flower.png) |
| 5 | Sugar | ![sugar](../images/ends_sugar.png) |

---
##  4}]
**ID Technique** : `[{'item': 'ends_delight:chorus_fruit_pie_slice', 'count': 4}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit Pie | ![chorus_fruit_pie](../images/ends_chorus_fruit_pie.png) |

---
## Chorus Fruit Popsicle
**ID Technique** : `ends_delight:chorus_fruit_popsicle` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Chorus Fruit Grain | ![chorus_fruit_grain](../images/ends_chorus_fruit_grain.png) |
| 1 | Ice | ![ice](../images/ends_ice.png) |
| 2 | Stick | ![stick](../images/ends_stick.png) |

---
## Chorus Fruit Wine
**ID Technique** : `ends_delight:chorus_fruit_wine` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Sugar | ![sugar](../images/ends_sugar.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |

---
##  1}]
**ID Technique** : `[{'item': 'ends_delight:liquid_dragon_egg', 'count': 1}, {'item': 'ends_delight:dragon_egg_shell', 'count': 1}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Non Hatchable Dragon Egg | ![non_hatchable_dragon_egg](../images/ends_non_hatchable_dragon_egg.png) |

---
## Dragon Breath And Chorus Soup
**ID Technique** : `ends_delight:dragon_breath_and_chorus_soup` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Suspicious Stew | ![suspicious_stew](../images/ends_suspicious_stew.png) |
| - | Dragon Breath | ![dragon_breath](../images/ends_dragon_breath.png) |
| - | Chorus Flower | ![chorus_flower](../images/ends_chorus_flower.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |

---
## Dragon Breath Soda
**ID Technique** : `ends_delight:dragon_breath_soda` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Dragon Breath | ![dragon_breath](../images/ends_dragon_breath.png) |
| - | Sugar | ![sugar](../images/ends_sugar.png) |

---
## Dragon Egg Shell Knife
**ID Technique** : `ends_delight:dragon_egg_shell_knife` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Dragon Egg Shell | ![dragon_egg_shell](../images/ends_dragon_egg_shell.png) |
| 1 | Popped Chorus Fruit | ![popped_chorus_fruit](../images/ends_popped_chorus_fruit.png) |

---
## Dragon Leg With Sauce Block
**ID Technique** : `ends_delight:dragon_leg_with_sauce_block` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| 1 | Ender Sauce | ![ender_sauce](../images/ends_ender_sauce.png) |
| 2 | Crimson Fungus | ![crimson_fungus](../images/ends_crimson_fungus.png) |
| 3 | Smoked Dragon Leg | ![smoked_dragon_leg](../images/ends_smoked_dragon_leg.png) |
| 4 | Warped Fungus | ![warped_fungus](../images/ends_warped_fungus.png) |
| 5 | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |

---
## Dragon Meat Stew Block
**ID Technique** : `ends_delight:dragon_meat_stew_block` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Raw Dragon Meat | ![raw_dragon_meat](../images/ends_raw_dragon_meat.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Carrot | ![carrot](../images/ends_carrot.png) |
| - | Potato | ![potato](../images/ends_potato.png) |
| - | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |

---
## Dragon Tooth Knife
**ID Technique** : `ends_delight:dragon_tooth_knife` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Dragon Tooth | ![dragon_tooth](../images/ends_dragon_tooth.png) |
| 1 | Popped Chorus Fruit | ![popped_chorus_fruit](../images/ends_popped_chorus_fruit.png) |

---
## Dried Chorus Flower
**ID Technique** : `ends_delight:dried_chorus_flower` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Chorus Flower | ![chorus_flower](../images/ends_chorus_flower.png) |

---
## Dried Endermite Meat
**ID Technique** : `ends_delight:dried_endermite_meat` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Ender Mite Meat | ![raw_ender_mite_meat](../images/ends_raw_ender_mite_meat.png) |

---
## Ender Congee
**ID Technique** : `ends_delight:ender_congee` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Crops/Rice | ![crops/rice](../images/ends_crops/rice.png) |
| - | Dried Endermite Meat | ![dried_endermite_meat](../images/ends_dried_endermite_meat.png) |
| - | Ender Pearl Grain | ![ender_pearl_grain](../images/ends_ender_pearl_grain.png) |

---
##  4}]
**ID Technique** : `[{'item': 'ends_delight:ender_pearl_grain', 'count': 4}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Ender Pearl | ![ender_pearl](../images/ends_ender_pearl.png) |

---
## Ender Sauce
**ID Technique** : `ends_delight:ender_sauce` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |

---
## End Barbecue Stick
**ID Technique** : `ends_delight:end_barbecue_stick` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Roasted Shulker Meat | ![roasted_shulker_meat](../images/ends_roasted_shulker_meat.png) |
| - | Roasted Dragon Meat | ![roasted_dragon_meat](../images/ends_roasted_dragon_meat.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Dried Endermite Meat | ![dried_endermite_meat](../images/ends_dried_endermite_meat.png) |
| - | Stick | ![stick](../images/ends_stick.png) |
| - | Stick | ![stick](../images/ends_stick.png) |
| - | Stick | ![stick](../images/ends_stick.png) |

---
## End Mixed Salad
**ID Technique** : `ends_delight:end_mixed_salad` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Crops/Cabbage | ![crops/cabbage](../images/ends_crops/cabbage.png) |
| - | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| - | Bowl | ![bowl](../images/ends_bowl.png) |

---
## End Stone Knife
**ID Technique** : `ends_delight:end_stone_knife` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | End Stone | ![end_stone](../images/ends_end_stone.png) |
| 1 | Popped Chorus Fruit | ![popped_chorus_fruit](../images/ends_popped_chorus_fruit.png) |

---
## End Stove
**ID Technique** : `ends_delight:end_stove` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 1 | Popped Chorus Fruit | ![popped_chorus_fruit](../images/ends_popped_chorus_fruit.png) |
| 2 | End Stone Bricks | ![end_stone_bricks](../images/ends_end_stone_bricks.png) |
| 3 | Campfire | ![campfire](../images/ends_campfire.png) |

---
## Fried Dragon Egg
**ID Technique** : `ends_delight:fried_dragon_egg` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Liquid Dragon Egg | ![liquid_dragon_egg](../images/ends_liquid_dragon_egg.png) |

---
## Fried Dragon Egg
**ID Technique** : `ends_delight:fried_dragon_egg` | **Machine** : `minecraft:campfire_cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Liquid Dragon Egg | ![liquid_dragon_egg](../images/ends_liquid_dragon_egg.png) |

---
## Fried Dragon Egg
**ID Technique** : `ends_delight:fried_dragon_egg` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Liquid Dragon Egg | ![liquid_dragon_egg](../images/ends_liquid_dragon_egg.png) |

---
## Grilled Shulker Block
**ID Technique** : `ends_delight:grilled_shulker_block` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 1 | Roasted Shulker Meat | ![roasted_shulker_meat](../images/ends_roasted_shulker_meat.png) |
| 2 | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| 3 | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| 4 | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| 5 | Crops/Onion | ![crops/onion](../images/ends_crops/onion.png) |
| 6 | Shulker Shell | ![shulker_shell](../images/ends_shulker_shell.png) |

---
## Half Dragon Egg Shell
**ID Technique** : `ends_delight:half_dragon_egg_shell` | **Machine** : `crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Obsidian | ![obsidian](../images/ends_obsidian.png) |
| 1 | Dragon Egg Shell | ![dragon_egg_shell](../images/ends_dragon_egg_shell.png) |

---
##  6}]
**ID Technique** : `[{'item': 'ends_delight:raw_dragon_meat'}, {'item': 'minecraft:bone_meal', 'count': 6}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Dragon Leg | ![dragon_leg](../images/ends_dragon_leg.png) |

---
## Non Hatchable Dragon Egg
**ID Technique** : `ends_delight:non_hatchable_dragon_egg` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Obsidian | ![obsidian](../images/ends_obsidian.png) |
| 1 | End Crystal | ![end_crystal](../images/ends_end_crystal.png) |
| 2 | Dragon Tooth | ![dragon_tooth](../images/ends_dragon_tooth.png) |

---
## Purpur Knife
**ID Technique** : `ends_delight:purpur_knife` | **Machine** : `minecraft:crafting_shaped`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| 0 | Popped Chorus Fruit | ![popped_chorus_fruit](../images/ends_popped_chorus_fruit.png) |

---
##  3}]
**ID Technique** : `[{'item': 'ends_delight:raw_dragon_meat_cuts', 'count': 3}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Raw Dragon Meat | ![raw_dragon_meat](../images/ends_raw_dragon_meat.png) |

---
## Roasted Dragon Meat
**ID Technique** : `ends_delight:roasted_dragon_meat` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat | ![raw_dragon_meat](../images/ends_raw_dragon_meat.png) |

---
## Roasted Dragon Meat
**ID Technique** : `ends_delight:roasted_dragon_meat` | **Machine** : `minecraft:campfire_cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat | ![raw_dragon_meat](../images/ends_raw_dragon_meat.png) |

---
## Roasted Dragon Meat Cuts
**ID Technique** : `ends_delight:roasted_dragon_meat_cuts` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat Cuts | ![raw_dragon_meat_cuts](../images/ends_raw_dragon_meat_cuts.png) |

---
## Roasted Dragon Meat Cuts
**ID Technique** : `ends_delight:roasted_dragon_meat_cuts` | **Machine** : `minecraft:campfire_cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat Cuts | ![raw_dragon_meat_cuts](../images/ends_raw_dragon_meat_cuts.png) |

---
## Roasted Dragon Meat Cuts
**ID Technique** : `ends_delight:roasted_dragon_meat_cuts` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat Cuts | ![raw_dragon_meat_cuts](../images/ends_raw_dragon_meat_cuts.png) |

---
## Roasted Dragon Meat
**ID Technique** : `ends_delight:roasted_dragon_meat` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Raw Dragon Meat | ![raw_dragon_meat](../images/ends_raw_dragon_meat.png) |

---
## Roasted Dragon Steak
**ID Technique** : `ends_delight:roasted_dragon_steak` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Ender Sauce | ![ender_sauce](../images/ends_ender_sauce.png) |
| - | Roasted Dragon Meat | ![roasted_dragon_meat](../images/ends_roasted_dragon_meat.png) |
| - | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| - | Warped Fungus | ![warped_fungus](../images/ends_warped_fungus.png) |
| - | Crimson Fungus | ![crimson_fungus](../images/ends_crimson_fungus.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Bowl | ![bowl](../images/ends_bowl.png) |

---
## Roasted Shulker Meat
**ID Technique** : `ends_delight:roasted_shulker_meat` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |

---
## Roasted Shulker Meat
**ID Technique** : `ends_delight:roasted_shulker_meat` | **Machine** : `minecraft:campfire_cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |

---
## Roasted Shulker Meat Slice
**ID Technique** : `ends_delight:roasted_shulker_meat_slice` | **Machine** : `minecraft:smelting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat Slice | ![shulker_meat_slice](../images/ends_shulker_meat_slice.png) |

---
## Roasted Shulker Meat Slice
**ID Technique** : `ends_delight:roasted_shulker_meat_slice` | **Machine** : `minecraft:campfire_cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat Slice | ![shulker_meat_slice](../images/ends_shulker_meat_slice.png) |

---
## Roasted Shulker Meat Slice
**ID Technique** : `ends_delight:roasted_shulker_meat_slice` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat Slice | ![shulker_meat_slice](../images/ends_shulker_meat_slice.png) |

---
## Roasted Shulker Meat
**ID Technique** : `ends_delight:roasted_shulker_meat` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |

---
##  2}]
**ID Technique** : `[{'item': 'ends_delight:shulker_meat_slice', 'count': 2}]` | **Machine** : `farmersdelight:cutting`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |

---
## Smoked Dragon Leg
**ID Technique** : `ends_delight:smoked_dragon_leg` | **Machine** : `minecraft:smoking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| item | Dragon Leg | ![dragon_leg](../images/ends_dragon_leg.png) |

---
## Steamed Dragon Egg Block
**ID Technique** : `ends_delight:steamed_dragon_egg_block` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Liquid Dragon Egg | ![liquid_dragon_egg](../images/ends_liquid_dragon_egg.png) |
| - | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |
| - | Chorus Fruit | ![chorus_fruit](../images/ends_chorus_fruit.png) |
| - | Brown Mushroom | ![brown_mushroom](../images/ends_brown_mushroom.png) |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |
| - | Crops/Onion | ![crops/onion](../images/ends_crops/onion.png) |

---
## Stir Fried Shulker Meat
**ID Technique** : `ends_delight:stir_fried_shulker_meat` | **Machine** : `farmersdelight:cooking`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Crops/Onion | ![crops/onion](../images/ends_crops/onion.png) |
| - | Crops/Tomato | ![crops/tomato](../images/ends_crops/tomato.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |
| - | Shulker Meat | ![shulker_meat](../images/ends_shulker_meat.png) |

---
## Stuffed Rice Cake
**ID Technique** : `ends_delight:stuffed_rice_cake` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Crops/Rice | ![crops/rice](../images/ends_crops/rice.png) |
| - | Sugar | ![sugar](../images/ends_sugar.png) |
| - | Chorus Fruit Grain'}] | ![chorus_fruit_grain'}]](../images/ends_chorus_fruit_grain'}].png) |

---
## Purple Dye
**ID Technique** : `minecraft:purple_dye` | **Machine** : `minecraft:crafting_shapeless`

| Emplacement | Ingrédient | Icône |
| :--- | :--- | :--- |
| - | Chorus Succulent | ![chorus_succulent](../images/ends_chorus_succulent.png) |

---
